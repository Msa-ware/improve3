    ...
    : IUserCommandService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (user == null || !hashingService.VerifyPassword(command.Password, user.PasswordHash))
            return Result<(User user, string token)>.Failure(IamError.InvalidCredentials,
                _localizer[$"{nameof(IamError)}.{IamError.InvalidCredentials}"]);
    ...
    public async Task<Result> Handle(SignUpCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure(IamError.OperationCancelled, _localizer[$"{nameof(IamError)}.{IamError.OperationCancelled}"]);
        }
        catch (DbUpdateException)
        {
            // Log the exception details here if an ILogger is injected
            return Result.Failure(IamError.DatabaseError, _localizer[$"{nameof(IamError)}.{IamError.DatabaseError}"]);
        }
        catch (Exception)
        {
            // Log the exception details here if an ILogger is injected
            return Result.Failure(IamError.InternalServerError, _localizer[$"{nameof(IamError)}.{IamError.InternalServerError}"]);
        }
    }
}